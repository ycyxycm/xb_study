  ...
  <div class="wrap">
    <el-card>
      ...
      <el-table v-loading="loading" :data="tableData" border stripe style="width: 100%" :row-style="{height:'20px'}" :cell-style="{padding:'4px'}">
        ...
        <el-table-column prop="order_number" label="原始订单数量"></el-table-column>
        <el-table-column prop="send_refund_cost" label="发货后退款成本"></el-table-column>
      </el-table>
      <el-pagination
...
export default {
  ...
  methods: {
    requestData() {
      return request.get(api.http+"/dy/sendgoods").then(res => {
    ...
    onSubmit() {
      ...
      this.loading=true
      return request.get(api.http+"/dy/sendgoods",{params:this.form}).then(res => {
        if (res.data.status!=200){
          this.$notify.error({
            title: '错误',
            message: res.data.msg
