  ...
  <div class="wrap">
    ...
    <el-card>
      ...
      >
        <el-table-column width="40px" type="expand">
          <template slot-scope="props">
            <el-table
              ...
              <el-table-column width="125px" label="成交金额" prop="i_deal_money">
                <template slot-scope="scope">
                  <p>{{scope.row.i_deal_money|filterNumber}}</p>
                </template>
              </el-table-column>
              <el-table-column width="125px" label="退款金额" prop="i_refund_money">
                <template slot-scope="scope">
                  <p>{{scope.row.i_refund_money|filterNumber}}</p>
                </template>
              </el-table-column>
              <el-table-column width="125px" prop="i_budan_money" label="补单金额">
                <template slot-scope="scope">
                  <p>{{scope.row.i_budan_money|filterNumber}}</p>
                </template>
              </el-table-column>
              <el-table-column width="125px" prop="i_net_sale" label="净销售额">
                <template slot-scope="scope">
                  <p>{{scope.row.i_net_sale|filterNumber}}</p>
                </template>
              </el-table-column>
              <el-table-column width="125px" prop="i_tg_pay_count" label="付费推广合计">
                <template slot-scope="scope">
                  <p>{{scope.row.i_tg_pay_count|filterNumber}}</p>
                </template>
              </el-table-column>
              <el-table-column width="125px" prop="i_plat_fee_count" label="平台扣费合计">
                <template slot-scope="scope">
                  <p>{{scope.row.i_plat_fee_count|filterNumber}}</p>
                </template>
              </el-table-column>
              <el-table-column width="125px" prop="i_kf_fee_count" label="客服费用合计">
                <template slot-scope="scope">
                  <p>{{scope.row.i_kf_fee_count|filterNumber}}</p>
                </template>
              </el-table-column>
              <el-table-column width="125px" prop="i_net_ml_11" label="1.1净毛利">
                <template slot-scope="scope">
                  <p>{{scope.row.i_net_ml_11|filterNumber}}</p>
                </template>
              </el-table-column>
              <el-table-column width="125px" prop="i_net_ml_rate_11" label="1.1净毛利率"></el-table-column>
              <el-table-column width="125px" prop="i_refund_rate" label="退款率"></el-table-column>
              <el-table-column width="125px" prop="i_tg_pat_rate" label="运营付费占比"></el-table-column>
            </el-table>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="i_net_ml_11" width="125px" label="1.1净毛利"></el-table-column>
  ...
  data() {
    return {
      json_fields:{
        ...
        "客服费用合计":"i_kf_fee_count","1.1净毛利":"i_net_ml_11",
        "1.1净毛利率":"i_net_ml_rate_11","退款率":"i_refund_rate",
        "运营付费占比":"i_tg_pat_rate",
      },
      cur_data:[],
      //分页数据
      query: {
      ...
      this.data_all.forEach(function (item){
        if(!cur_map.hasOwnProperty(item.shop)){
          cur_map[item.shop]={
            ...
            'i_budan_money':item.i_budan_money,'i_net_sale':item.i_net_sale,'i_tg_pay_count':item.i_tg_pay_count,
            'i_plat_fee_count':item.i_plat_fee_count,'i_kf_fee_count':item.i_kf_fee_count, 'i_net_ml_11':item.i_net_ml_11,
            'i_net_ml_rate_11':item.i_net_ml_rate_11,
            'i_refund_rate':item.i_refund_rate,
            'i_tg_pat_rate':item.i_tg_pat_rate,
            'items':[item]
          }
        }else {
