<template>
  ...
  <div class="wrap">
    ...
    <el-card>
      <el-table v-loading="table_status" height="1000" :data="tableData" border stripe style="width: 100%"
...
<!--        <el-table-column fixed prop="now_date" label="日期" width="100"></el-table-column>-->
        ...
        <el-table-column fixed prop="i_net_ml_10_count" label="1.0净毛利">
          <template slot-scope="scope">
            <p>{{scope.row.i_net_ml_10_count|filterNumber}}</p>
          </template>
        </el-table-column>
        <el-table-column fixed prop="i_net_ml_rate_10_count" label="1.0净毛利率"></el-table-column>
        <el-table-column
  ...
  watch:{
    ...
    data_all(newV,oldV){
      ...
      this.data_all.forEach(function (item){
        ...
        if (!cur_map[item.shop].hasOwnProperty('i_net_ml_10_count')){
          cur_map[item.shop]['i_net_ml_10_count']=0
        }
        if (!cur_map[item.shop].hasOwnProperty('i_net_sale_count')){
          cur_map[item.shop]['i_net_sale_count']=0
        }
        cur_map[item.shop]['i_net_ml_10_count']+=item.i_net_ml_10
        cur_map[item.shop]['i_net_sale_count']+=item.i_net_sale
        if (cur_map[item.shop]['i_net_ml_10_count']==0||cur_map[item.shop]['i_net_sale_count']==0){
          cur_map[item.shop]['i_net_ml_rate_10_count']="0%"
        }else {
          cur_map[item.shop]['i_net_ml_rate_10_count']=(Math.round((cur_map[item.shop]['i_net_ml_10_count']/cur_map[item.shop]['i_net_sale_count'])*10000)/100).toString()+"%"
        }
        cur_map[item.shop][item.now_date]=Math.round(item.i_net_ml_10*100)/100



        cur_map[item.shop]['i_net_ml_10_count']=Math.round(cur_map[item.shop]['i_net_ml_10_count']*100)/100
      ...
      for(let key in cur_map){
        ...
        // cur2_map['shop']=key
        cur_data2.unshift(cur2_map)
      }
      this.cur_data=cur_data2

      this.json_fields={}
      for(let i in this.cur_data[0]){
        if (i==="i_net_ml_10_count"){
          var k="1.0净毛利 合计"
        }else if(i==="i_net_sale_count"){
          var k="净销售额 合计"
        }else if(i==="i_net_ml_rate_10_count"){
          var k="1.0净毛利率 合计"
        }else if(i==="shop"){
          var k="店铺"
        }else{
