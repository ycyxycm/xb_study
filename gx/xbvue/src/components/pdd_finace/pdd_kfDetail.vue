  ...
  <div class="wrap">
    ...
    <el-card>
      <el-table v-loading="table_status" :data="tableData" border stripe style="width: 100%" :row-style="{height:'20px'}"
        ...
        <el-table-column prop="i_kf_fee_count" label="客服费用合计">
          <template slot-scope="scope">
            <p>{{scope.row.i_kf_fee_count|filterNumber}}</p>
          </template>
        </el-table-column>
        <el-table-column prop="i_kffx" label="客服返现">
          <template slot-scope="scope">
            <p>{{scope.row.i_kffx|filterNumber}}</p>
          </template>
        </el-table-column>
        <el-table-column prop="i_sale_after_fee" label="售后费用">
          <template slot-scope="scope">
            <p>{{scope.row.i_sale_after_fee|filterNumber}}</p>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
export default {
  ...
  data() {
    return {
      json_fields:{
        "店铺":"shop","日期":"now_date",
        "客服费用合计":"i_kf_fee_count","客服返现":"i_kffx",
        "售后费用":"i_sale_after_fee"
      },
      //分页数据
      query: {
        pageNum: 1,
