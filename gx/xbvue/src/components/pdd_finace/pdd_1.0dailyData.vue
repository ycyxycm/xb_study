  ...
  <div class="wrap">
    ...
    <el-card>
      ...
      >
        <el-table-column width="40px" type="expand">
          <template slot-scope="props">
            <el-table
              ...
              <el-table-column prop="i_clothing_cost_10" width="125px" label="1.0衣服成本">
                <template slot-scope="scope">
                  <p>{{scope.row.i_clothing_cost_10|filterNumber}}</p>
                </template>
              </el-table-column>
              ...
              <el-table-column prop="i_ml_10" width="125px" label="1.0毛利">
                <template slot-scope="scope">
                  <p>{{scope.row.i_ml_10|filterNumber}}</p>
                </template>
              </el-table-column>
              ...
              <el-table-column prop="i_net_ml_10" width="125px" label="1.0净毛利">
                <template slot-scope="scope">
                  <p>{{scope.row.i_net_ml_10|filterNumber}}</p>
                </template>
              </el-table-column>
              <el-table-column prop="i_net_ml_rate_10" width="125px" label="1.0净毛利率"></el-table-column>
        ...
        <el-table-column :show-overflow-tooltip="true" label="店铺名称" width="300px" prop="shop"></el-table-column>
        <el-table-column label="成交金额" width="125px" prop="i_deal_money">
          <template slot-scope="scope">
            <p>{{scope.row.i_deal_money|filterNumber}}</p>
          </template>
  ...
  data() {
    return {
      json_fields:{
        ...
        "1.0衣服成本":"i_clothing_cost_10","快递费用":"i_kuaidi_fee",
        "发货费用":"i_sendgoods_fee","1.0毛利":"i_ml_10",
        "付费推广合计":"i_tg_pay_count","平台扣费合计":"i_plat_fee_count",
        "客服费用合计":"i_kf_fee_count","活动费用":"i_huodong_fee",
        "吊牌费":"i_tag_fee","1.0净毛利":"i_net_ml_10",
        "1.0净毛利率":"i_net_ml_rate_10","运营付费占比":"i_tg_pat_rate",
        "退款率":"i_refund_rate"
      },
      cur_data:[],
      //分页数据
      ...
      this.data_all.forEach(function (item){
        if(!cur_map.hasOwnProperty(item.shop)){
          cur_map[item.shop]={
            ...
            'i_budan_money':item.i_budan_money,'i_net_sale':item.i_net_sale,'i_clothing_cost_10':item.i_clothing_cost_10,
            'i_kuaidi_fee':item.i_kuaidi_fee,"i_sendgoods_fee":item.i_sendgoods_fee,"i_ml_10":item.i_ml_10,
            "i_tg_pay_count":item.i_tg_pay_count,"i_plat_fee_count":item.i_plat_fee_count,"i_kf_fee_count":item.i_kf_fee_count,
            "i_huodong_fee":item.i_huodong_fee,"i_tag_fee":item.i_tag_fee,"i_net_ml_10":item.i_net_ml_10,
            "i_net_ml_rate_10":item.i_net_ml_rate_10,"i_tg_pat_rate":item.i_tg_pat_rate,"i_refund_rate":item.i_refund_rate,
            'items':[item]
          }
        }else {
          ...
          cur_map[item.shop]['i_net_ml_10']+=item.i_net_ml_10

          cur_map[item.shop]['i_net_ml_rate_10']+=item.i_net_ml_rate_10
          ...
          if (cur_map[item.shop]['i_net_ml_10']===0||cur_map[item.shop]['i_net_sale']===0){
            var number1=0
          }else {
            var number1=Math.round((cur_map[item.shop]['i_net_ml_10']/cur_map[item.shop]['i_net_sale'])*10000)/100
          }
          ...
          cur_map[item.shop]['i_net_ml_rate_10']=number1.toString()+"%"//1.1净毛利/净销售额
          ...
          cur_map[item.shop]['i_clothing_cost_10']=Math.round(cur_map[item.shop]['i_clothing_cost_10']*100)/100
          ...
          cur_map[item.shop]['i_ml_10']=Math.round(cur_map[item.shop]['i_ml_10']*100)/100
          ...
          cur_map[item.shop]['i_net_ml_10']=Math.round(cur_map[item.shop]['i_net_ml_10']*100)/100
