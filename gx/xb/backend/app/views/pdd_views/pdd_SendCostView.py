...
#序列化器
class pddSendCostSerializer(serializers.Serializer):
    ...
    today_tag = serializers.DecimalField(max_digits=20,decimal_places=4,required=False)
    is_delete = serializers.IntegerField(default=0, required=False, write_only=True)  # 1=已删除 0=未删除

#ViewSet
class pddSendCostViewSet(APIView):
    def get(self, request):
        ...
        try:
            ...
            obj = models.pdd_sendcost.objects.filter(**parameters).all().order_by('-now_date')
            obj_ser = pddSendCostSerializer(instance=obj, many=True)
